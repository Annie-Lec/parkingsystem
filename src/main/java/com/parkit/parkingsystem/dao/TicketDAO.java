...
public class TicketDAO {
	...
	public int getNumberOfTicketAlreadyPaid(String vehicleRegNumber) {
		Connection con = null;
		int numberOfTicketAlreadyPaid = 0;
		try {
			con = dataBaseConfig.getConnection();
			PreparedStatement ps = con.prepareStatement(DBConstants.NUMBER_OF_TICKET_PAID_PER_USER);
			// ID, PARKING_NUMBER, VEHICLE_REG_NUMBER, PRICE, IN_TIME, OUT_TIME)
			ps.setString(1, vehicleRegNumber);
			ResultSet rs = ps.executeQuery();
			rs.next();
			numberOfTicketAlreadyPaid = rs.getInt("count(*)");

			dataBaseConfig.closeResultSet(rs);
			dataBaseConfig.closePreparedStatement(ps);
		} catch (Exception ex) {
			logger.error("Error counting ticket already paid", ex);
		} finally {
			dataBaseConfig.closeConnection(con);
		}
